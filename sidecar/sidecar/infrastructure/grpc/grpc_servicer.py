...
class TranscriptionServiceServicer(stt_pb2_grpc.TranscriptionServiceServicer):
    def __init__(
        ...
        self._transcription_service = TranscriptionService(engine_manager, executor)
    ...
    ) -> list[stt_pb2.ServerMessage]:
        messages = []
        for event in pipeline.process_audio(audio):
            if isinstance(event, SpeechStarted):
                messages.append(MessageMapper.to_grpc_speech_started())
                session.start_speech()
            elif isinstance(event, SpeechStopped):
                messages.append(MessageMapper.to_grpc_speech_stopped())
                session.stop_speech()
            elif isinstance(event, Transcript):
                messages.append(MessageMapper.to_grpc_transcript(event, is_partial=False))
        return messages

    async def _handle_active_session_async(
        self,
        session: SpeechSession,
        audio,
        ...
        if domain_config.partials:
            try:
                transcript = await partial_service.generate_partial_async(session, domain_config)
                if transcript:
                    messages.append(MessageMapper.to_grpc_transcript(transcript, is_partial=True))
            except Exception as e:
                messages.append(MessageMapper.to_grpc_error(e))
        return messages

    async def Transcribe(
        self, request_iterator: AsyncIterator[stt_pb2.ClientMessage], context
    ) -> AsyncIterator[stt_pb2.ServerMessage]:
        ...
        async for client_msg in request_iterator:
            ...
            if msg_type == "config":
                ...
                partial_service = PartialTranscriptService(
                    transcribe_fn=self._transcription_service.transcribe_with_retry,
                    transcribe_async_fn=self._transcription_service.transcribe_async,
                )
            ...
            if msg_type == "audio":
                ...
                for msg in self._process_pipeline_events(pipeline, audio, session):
                    yield msg
                for msg in await self._handle_active_session_async(session, audio, partial_service, domain_config):
                    yield msg

            elif msg_type == "encoded_audio":
                try:
                    encoded_msg = client_msg.encoded_audio
                    transcript = await self._transcription_service.transcribe_encoded_async(
        ...
        if pipeline is not None and partial_service is not None:
            ...
            if remaining_audio is not None and len(remaining_audio) > 0:
                try:
                    ...
                    if duration_ms > 0:
                        transcript = await self._transcription_service.transcribe_async(audio=remaining_audio)
                        yield MessageMapper.to_grpc_transcript(transcript, is_partial=False)
                except Exception as e:
                    yield MessageMapper.to_grpc_error(e)
            pipeline.reset()
